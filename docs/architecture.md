...
commands used by hand-written scripts (`set`, `add`, `multiply`, `divide`,
`unset`, `toggle`) and are executed after the matching text script. This keeps a
future Scratch-like block editor on top of the same runtime semantics instead
of introducing a second scripting engine.
